...
The HTMLTags module defines a class for all the valid HTML tags, written in
 uppercase letters. To create a piece of HTML, the general syntax will be :
...
If you have a list of instances, you can't concatenate the items with 
...
class TAG:
    ...
    def __init__(self, innerHTML="", **attrs):
        ...
        self.innerHTML = [str(innerHTML).lower()]
        self.attrs = [attrs]
    ...
    def __str__(self):
        ...
        for tag, innerHTML, attrs in zip(self.tag, 
            ...
            if not tag=='TEXT':
                w("<%s" %tag)
                attr1 = [ k for k in attrs if attrs[k] is not True ]
...
# create the classes
for tag in ClosingTags + NonClosingTags :
    exec("class %s(TAG): pass" %tag)
    
class TEXT(TAG):
    """Required to enter raw text"""
    pass
    
def Sum(aList):
    """Return the concatenation of the instances in aList
